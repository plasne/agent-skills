...
When the user asks for a larger workflow that includes demo-data generation, do
not merge the data work into unrelated deployment or pipeline tasks. Keep data
generation as its own delegated phase.
...
- Keep the main thread in a coordinator role. Use sub-agents for execution,
  especially for large data generation runs, API import loops, and validation.
- Write or update concise artifacts in the session workspace when available
  (for example: generated JSON files, import summaries, counts, and next-step
  notes). Prefer artifact handoff over repeating all state in the conversation.
...
Each permutation contains `--results` results (default 300). Each result has:
