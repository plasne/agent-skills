...
Recommended phases for long GTC work:

1. discovery and plan
2. Cosmos and hosting infrastructure
3. authentication and app registration setup
...
- Keep the main thread in a coordinator role. Use sub-agents for execution,
  especially for image builds, Azure provisioning, auth debugging, data
  seeding, and retry/debug loops.
- After each phase, write or update concise artifacts in the session workspace
  when available (for example: env files, auth summaries, app IDs, URLs, data
  summaries, and next-step notes). Prefer artifact handoff over repeating all
...
- Before moving to the next phase, checkpoint the current live state: resource
  names, app IDs, URLs, artifact paths, verified auth behavior, and unresolved
  blockers.
...
> [!IMPORTANT]
> For Azure Container Apps deployments, the GTC backend container listens on
> port `8080`. If the Container App ingress target port is set to `8000`, the
> app may appear deployed but requests can hang or fail even though the revision
> is running.
...
- `allowedAudiences` must include `api://<appId>` (the audience requested from `az account get-access-token --resource api://<appId>`)
- In practice, Azure CLI tokens often carry the raw app ID in the `aud` claim even when `api://<appId>` was requested. Include **both** `<appId>` and `api://<appId>` in `allowedAudiences` to avoid false `401` failures.
...
> [!IMPORTANT]
> For Container Apps auth config, the `2025-07-01` ARM API surface is more
> reliable than `2024-03-01` when patching the full `identityProviders` payload.
> If provider settings appear to disappear after a PUT, retry with the newer API
> version.
...
- **Deployed GTC returns `500` on "Request More"**: If Cosmos DB has `publicNetworkAccess: Disabled`, the Container App may be blocked by the Cosmos firewall. Either enable public network access and allowlist the Container App outbound IPs, or deploy private networking explicitly. Check the live logs for `Request originated from IP ... This is blocked by your Cosmos DB account firewall settings`.
...
- **EasyAuth configured but interactive login still fails on Container Apps**: Ensure the Container App has a **system-assigned managed identity** and a blob-backed token store enabled. The token store also requires `Storage Blob Data Contributor` on the backing storage account for the Container App system identity.
