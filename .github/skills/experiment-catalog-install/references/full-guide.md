...
Recommended phases for long catalog work:

1. discovery and plan
2. infrastructure and storage provisioning
3. authentication and app registration setup
4. application deployment and verification
5. data loading or integration setup
...
- Keep the main thread in a coordinator role. Use sub-agents for execution,
  especially for builds, Azure provisioning, auth debugging, data generation,
  and retry/debug loops.
- After each phase, write or update concise artifacts in the session workspace
  when available (for example: env files, JSON summaries, app IDs, URLs,
  role-assignment details, and next-step notes). Prefer artifact handoff over
...
- When creating both a delegated scope and a pre-authorized application entry, create the scope first and then add preauthorization. Microsoft Graph rejects payloads that reference a delegated permission ID before that scope exists on the app registration.
...
| `Catalog.ReadWrite` app role appears missing after app registration setup               | App role patch did not persist or was never created                         | Verify with `az ad app show --id <appId> --query 'appRoles' -o json`. If missing, patch the app registration via Graph to add an Application-type app role and then re-run the managed identity assignment.                                                 |
